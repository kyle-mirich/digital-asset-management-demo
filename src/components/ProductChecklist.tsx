...
import { ProductChecklistItem } from '@/types/product'
...
export default function ProductChecklist({ productId, onChecklistUpdate }: ProductChecklistProps) {
  const [checklistItems, setChecklistItems] = useState<ProductChecklistItem[]>([])
  ...
  if (!checklistItems.length) {
    return (
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-200 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Product Checklist</h3>
        <p className="text-sm text-gray-600">No checklist items found for this product.</p>
      </div>
    )
  }
    ...
    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-200 p-6">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-lg font-semibold text-gray-900">Product Checklist</h3>
          <span className={`
        ...
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div 
            className={`h-2 rounded-full transition-all duration-300 ${
              progress.progressPercentage === 100 ? 'bg-green-500' : 'bg-blue-500'
            }`}
            ...
            {optionalItems.map((item) => (
              ...
              >
                ...
                <div className="flex-1">
                  <p className={`
                    text-sm font-medium transition-all duration-200
                    ${item.is_completed ? 'text-blue-800 line-through' : 'text-gray-900'}
                  `}>
                    {item.title}
                  </p>
                  {item.description && (
                    <p className={`
                      text-xs mt-1 transition-all duration-200
                      ${item.is_completed ? 'text-blue-600' : 'text-gray-600'}
                    `}>
                      {item.description}
                    </p>
                  )}
        ...
        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-center">
            ...
            <div className="text-sm">
              ...
              <p className="text-green-600">This product is eligible for review status promotion.</p>
            </div>
          </div>
        </div>
      )}
